...
`paper/build/main.pdf`. Five empirical figures are generated by
`paper/figures/plot_empirical_figures.py` and catalogued in
`paper/figures/figure_catalog.json`. The lightweight audited summary is under
`results/dow30_serial/`; raw checkpoints and Monte Carlo arrays are excluded
from Git history because they are large and are regenerated by the formal
command.
